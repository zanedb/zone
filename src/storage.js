...
const { randid, validid, sanitizeDangerousHtml } = require('./utils.js')
...
class StorageBackend {
  ...
  has(id) {
    return new Promise((res, rej) => {
      if (!id) {
        ...
        fs.access(this.pathFromID(id), fs.constants.F_OK, err => {
          res(!err)
        })
      }
  ...
  get(id) {
    return new Promise((res, rej) => {
      fs.readFile(this.pathFromID(id), 'utf8', (err, data) => {
        if (err) {
          rej(err)
        } else {
...
class Record {
  ...
  getRedirect() {
    if (!this.isURI()) {
      throw new Error(
        `Cannot render a redirect path for record of type ${this.type}`
      )
    }
